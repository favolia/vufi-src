...
const createUserDataModel = data => {
  ...
  return {
    ...
    // important to store for fetching posts
...
const createUserVideosDataModel = data => {
  ...
  if (!data.itemList || !data.itemList.length) return {}; // find author info from first item
  ...
  return {
    ...
    secUid: firstItem.author.secUid,
    // important to store for fetching posts
...
module.exports.getUser = async (username, options = {}) => {
  const promise = new Promise(async (resolve, reject) => {
    try {
      ...
      const data = text.split(/<script id="__NEXT_DATA__" type="application\/json" nonce="[\w-]+" crossorigin="anonymous">/)[1].split(`</script>`)[0];
      resolve({
